...
/*
    admin/galeri.php
    ===================================================================
    Toolbar'daki "Sütun/Satır" ile içerik resmi eklerken VE okuma.php
    gibi sayfalarda kapak görseli seçerken açılan ortak galeri.

    hedef = hangi klasöre yükleneceği (uploads/{hedef}/ ya da düz uploads/)
    tip   = 'kapak' ise seçim window.opener.kapakResmiAl() ile,
            değilse window.opener.galeridenResmiAl() ile ana pencereye
            (main.js) bildirilir.
    ===================================================================
*/
...
$yuklemeKlasoru = in_array($hedef, $klasorluHedefler, true)
    ? __DIR__ . '/../uploads/' . $hedef . '/'
    : __DIR__ . '/../uploads/';
...
function resimKucult($kaynak, $hedefYol, $sabitYukseklik = 800, $kalite = 75) {
    ...
    switch ($tip) {
        ...
        case 'image/webp': $kaynakResim = imagecreatefromwebp($kaynak); break;
        default: return 'hata';
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['resim'])) {
    ...
    if (in_array($dosyaUzantisi, $izinliUzantilar, true)) {
        ...
        $hedefYol = $yuklemeKlasoru . $yeniDosyaAdi;

        $sonuc = resimKucult($dosya['tmp_name'], $hedefYol, 800, 75);

        if ($sonuc === 'basarili') {
            $mesaj = '<div class="galeri-mesaj galeri-mesaj-basarili"><i class="fa-solid fa-circle-check"></i> Resim başarıyla eklendi.</div>';
        ...
        <form action="" method="POST" enctype="multipart/form-data" class="galeri-yukleme-alani">
            <div>
                ...
                <div style="display: flex; align-items: center; gap: 10px; margin-top: 8px;">
                    ...
                    <!-- GİZLİ GERÇEK INPUT (name="resim" olarak kalmalı ki PHP okusun) -->
